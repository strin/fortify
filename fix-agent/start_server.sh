...
# Script to start the fix-agent server
...
# DATABASE_URL, ANTHROPIC_API_KEY, and GITHUB_TOKEN should be provided externally

# Generate Prisma client if needed (for production environments)
echo "Generating Prisma client..."
cd /db && npx prisma generate && cd /fix-agent
...
if [ "$NODE_ENV" = "production" ] || [ "$ENVIRONMENT" = "production" ] || [ -n "$RENDER" ]; then
    ...
    exec gunicorn fix_agent.server:app \
        --bind 0.0.0.0:$PORT \
