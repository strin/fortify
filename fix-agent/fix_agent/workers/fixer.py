...
class FixWorker:
    ...
    async def _process_next_job(self):
        ...
        try:
            # Claim a fix job (blocking with timeout)
            job = self.queue.claim_fix_job(timeout=5)
            ...
            try:
                # Process the fix job
                result = await self._execute_fix_job(job)
                
                if result:
                    # Mark job as completed
                    job.result = result
                    self.queue.complete_fix_job(job)
                    
                    # Update database
                    db_manager = await get_db_manager()
                    await db_manager.save_fix_result(
                        job.id,
                        result,
                        result.branchName,
                        result.commitSha,
                    ...
                    self.queue.fail_fix_job(job, "Fix processing failed")
                    logger.error(f"Fix job {job.id} failed")
                    
            except Exception as e:
                # Handle job processing errors
                error_msg = f"Fix job processing error: {str(e)}"
                self.queue.fail_fix_job(job, error_msg)
    ...
    async def _clone_repository(self, repo_url: str, branch: str, temp_dir: str) -> Optional[str]:
        """
    ...
    async def _generate_fix(self, job: FixJob, repo_path: str) -> Optional[str]:
        ...
        try:
            ...
            logger.info(f"Generating fix for {vulnerability.category} vulnerability in {vulnerability.filePath}")
    ...
    async def _apply_fix(self, job: FixJob, repo_path: str, fix_content: str) -> Optional[list]:
        """
    ...
    async def _commit_fix(self, job: FixJob, repo_path: str, modified_files: list) -> Optional[str]:
    ...
    async def _create_pull_request(self, job: FixJob, branch_name: str) -> tuple[Optional[str], Optional[int]]:
