...
import { Chat, Creator, Post, Profile } from "@/types";
...
const LiveTranscriptComponent = ({ creator }: { creator: Creator }) => {
  const [lastAssistantTranscript, setLastAssistantTranscript] =
    React.useState<string>("");
  const [openTranscript, setOpenTranscript] = useState(false);

  const { isConnected, lastTranscriptSegment, room } = useLiveKit();
  ...
  const handleInterrupt = () => {
    if (room) {
      // Send the interrupt event to the backend
      const data = {
...
const LiveCall: React.FC<{
  ...
  if (!post.prompt?.id) {
    return (
      <Alert variant="destructive">
        ...
        <AlertDescription>No prompt ID available.</AlertDescription>
      </Alert>
    );
  }

  console.log("live call", creator, creator.username, post);
  ...
  const ParticipantSquares = () => {
    return (
      <div className="w-full max-w-4xl grid grid-cols-2 gap-4 px-4">
          ...
          <div className="w-full h-full">
            ...
            {userImage ? (
              <img
                src={userImage}
                alt={user?.name || "User"}
                className="w-24 h-24 rounded-full object-cover absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2"
              />
            ) : (
              <div className="w-24 h-24 rounded-full bg-gray-600 flex items-center justify-center absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
                <svg
  ...
  const slideMode = post.coverImages && post.coverImages.length > 0;

  const SlidesView = () => {
    if (!post.coverImages || post.coverImages.length === 0) return null;

    return (
      <div className="w-full h-full flex flex-col items-center justify-center relative">
        {/* Fixed canvas-like screen for slides */}
        <div className="w-full max-w-4xl md:aspect-video aspect-square bg-black/10 flex items-center justify-center rounded-lg shadow-lg overflow-hidden">
          <img
          ...
          <div className="flex-grow flex items-center justify-center relative flex-col">
            <LiveKitProvider
              roomNamePrefix={`${sandboxPrefix}users/${
                user?.id || "-1"
              }/creators/${creator.id}/posts/${post.id}`}
            ...
            >
              ...
              >
                {slideMode ? <SlidesView /> : <ParticipantSquares />}
              </div>
